...
@onready var ray_cast_3d: ShapeCast3D = $"../ShapeCast3D"
...
func _on_pickable_object_action_pressed(pickable: Variant) -> void:
	...
	if not ammo == 0:
		if ray_cast_3d.is_colliding():
			var hit = ray_cast_3d.get_collider(0)
			hit.remove_from_group("Shootable")
			hit.queue_free()
			shoot_delay.start()
			ammo -= 1
			animation_player.play("gun fire")
			audio_stream_player_3d.play(1.2)
	else:
...
func _on_death_timeout() -> void:
	SceneSwich.scene_switch()
	TextSystem.text_change("test2")
	TextSystem.step_text()
	datamanger.player_dead = true
